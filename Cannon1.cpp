...
public:
    ...
    float white_town_attack(){
    	if(N==8 && M==8){
	        float utility1=0;
	        vector<float> town_hall_attack(white_T.size(),0);
	        for(int j=0;j<white_T.size();j++){
	            for(int i=0;i<black_S.size();i++){
	                town_hall_attack[j] += max(0,9-man_dis(black_S.get(i),white_T.get(j)));
	            }
	            utility1 += min(10,town_hall_attack[j]);
    ...
    float black_town_attack(){
    	if(N==8 && M==10){
	        float utility1=0;
	        vector<float> town_hall_attack(black_T.size(),0);
	        for(int j=0;j<black_T.size();j++){
	            for(int i=0;i<white_S.size();i++){
	                town_hall_attack[j] += max(0,9-man_dis(white_S.get(i),black_T.get(j)));
	            }                                                             
	            utility1 += min(10,town_hall_attack[j]);
    ...
    float black_town_safety(){
    	if(N==8 && M==8){
	        float utility1=0;
	        vector<float> town_hall_safety(black_T.size(),0);
	        for(int j=0;j<black_T.size();j++){
	            for(int i=0;i<black_S.size();i++){
	                town_hall_safety[j] +=  max(0,9-man_dis(black_S.get(i),black_T.get(j)));
	            }
	            utility1 += min(10,town_hall_safety[j]);
	        }
	        utility1 = utility1;
	        return utility1;
    	}
    ...
    float white_town_safety(){
    	if(N==8 && M==8){
	        float utility1=0;
	        vector<float> town_hall_safety(white_T.size(),0);
	        for(int j=0;j<white_T.size();j++){
	            for(int i=0;i<white_S.size();i++){
	                town_hall_safety[j] +=  max(0,9-man_dis(white_S.get(i),white_T.get(j)));
	            }
	            utility1 += min(10,town_hall_safety[j]);
	        }
	        utility1 = utility1;
    ...
    float utility(){
		    	...
		    	if(stratergy ==1){
			        ...
			        utility1 += (w[0]-w[2]*color)*black_S.size()-(w[0]+w[2]*color)*white_S.size();
			        // utility1 += w[3]*(Cannon_black.size()-Cannon_white.size());
			        utility1 += w[4]*(hit_from_Cannon_Black(1,2,2)-hit_from_Cannon_White(1,2,2));
			        utility1 += (w[5]-w[5]*color)*black_town_safety()-(w[5]+w[5]*color)*white_town_safety();
			        utility1 += w[1]*(black_T.size()-white_T.size());
				}
				else if(stratergy==3){
					utility1 += (w[0]-w[2]*mod)*black_S.size() -(w[0]+w[2]*mod)*white_S.size();
					// utility1 += ((w[3]-mod)*Cannon_black.size()-(w[3]+mod)*Cannon_white.size());
			        utility1 += w[4]*(hit_from_Cannon_Black(1,2,2)-hit_from_Cannon_White(1,2,2));
			       	utility1 += (w[5]-(w[5])*mod)*black_town_safety()-((w[5])+(w[5])*mod)*white_town_safety();
			        utility1 += (w[6]+w[6]*mod)*white_town_attack()-(w[6]-w[6]*mod)*black_town_attack();
			       	utility1 += w[1]*(black_T.size()-white_T.size());
				}
				else if(stratergy ==4){
					utility1 += (w[0]-w[2]*color)*black_S.size() -(w[0]+w[2]*color)*white_S.size();
					// utility1 += ((w[3])-color)*Cannon_black.size()-((w[3])+color)*Cannon_white.size();
			        utility1 += (w[4])*(hit_from_Cannon_Black(1,2,2)-hit_from_Cannon_White(1,2,2));
					...
					utility1 += (w[0]-w[2]*mod)*black_S.size() -(w[0]+w[2]*mod)*white_S.size();
					// utility1 += ((w[3]-mod)*Cannon_black.size()-(w[3]+mod)*Cannon_white.size());
			        utility1 += ((w[4])+mod)*(hit_from_Cannon_Black(1,2,2))-((w[4])-mod)*(hit_from_Cannon_White(1,2,2));
			        ...
			        utility1 += (w[7]+mod)*white_town_hall_attack_defence_utility()-(w[7]-mod)*black_town_hall_attack_defence_utility();
			       	utility1 += w[1]*(black_T.size()-white_T.size());
					...
					utility1 += (w[0]-w[2]*color)*black_S.size()-(w[0]+w[2]*color)*white_S.size();
					// utility1 += w[3]*(Cannon_black.size()-Cannon_white.size());
			        utility1 += w[4]*(hit_from_Cannon_Black(1,1,1)-hit_from_Cannon_White(1,1,1));
			        utility1 += ((w[7])-color)*white_town_hall_attack_defence_utility()-((w[7])+color)*black_town_hall_attack_defence_utility();
			       	utility1 += w[1]*(black_T.size()-white_T.size());
				}
				if(color==-1)
				...
				// 		float utility1 =0;
				// 			if(stratergy ==1){
				// 					cerr << (w[0]-w[2]*color)*black_S.size()-(w[0]+w[2]*color)*white_S.size() << endl;
				// 					cerr << (w[3]-color)*(Cannon_black.size())-(w[3]+color)*Cannon_white.size() << endl;
				// 					cerr << (w[4]-1.5*color)*(hit_from_Cannon_Black(0,0,0))-(w[4]+1.5*color)*(hit_from_Cannon_White(0,0,0)) << endl;
				// 					cerr << w[1]*(black_T.size()-white_T.size()) << endl;
				// 			}
				// 			else if(stratergy==2){
				// 					cerr << (w[0]-w[2]*color)*black_S.size()-(w[0]+w[2]*color)*white_S.size() << endl;
				// 					cerr << w[3]*(Cannon_black.size()-Cannon_white.size()) << endl;
				// 					cerr << w[4]*(hit_from_Cannon_Black()-hit_from_Cannon_White()) << endl;
				// 					cerr << (w[5]-w[5]*color)*black_town_safety()-(w[5]+w[5]*color)*white_town_safety() << endl;
				// 					cerr << w[1]*(black_T.size()-white_T.size()) << endl;
				// 		}
				// 		else if(stratergy==3){
				// 					cerr << (w[0]-w[2]*mod)*black_S.size() -(w[0]+w[2]*mod)*white_S.size() << endl;
				// 					cerr << ((w[3]-mod)*Cannon_black.size()-(w[3]+mod)*Cannon_white.size()) << endl;
				// 					cerr << w[4]*(hit_from_Cannon_Black()-hit_from_Cannon_White()) << endl;
				// 					cerr << (w[5]-(w[5])*mod)*black_town_safety()-((w[5])+(w[5])*mod)*white_town_safety() << endl;
				// 					cerr << (w[6]+w[6]*mod)*white_town_attack()-(w[6]-w[6]*mod)*black_town_attack() << endl;
				// 					cerr << w[1]*(black_T.size()-white_T.size()) << endl;
				// 		}
				// 		else if(stratergy ==4){
				// 					cerr << (w[0]-w[2]*color)*black_S.size() -(w[0]+w[2]*color)*white_S.size() << endl;
				// 					cerr << ((w[3])-color)*Cannon_black.size()-((w[3])+color)*Cannon_white.size() << endl;
				// 					cerr << (w[4])*(hit_from_Cannon_Black()-hit_from_Cannon_White()) << endl;
				// 					cerr << (w[7]-color)*(white_town_hall_attack_defence_utility())-(w[7]+color)*(black_town_hall_attack_defence_utility()) << endl;
				// 					cerr << w[1]*(black_T.size()-white_T.size()) << endl;
				// 		}
				// 		else if(stratergy ==5){
				// 					cerr << (w[0]-w[2]*mod)*black_S.size() -(w[0]+w[2]*mod)*white_S.size() << endl;
				// 					cerr << ((w[3]-mod)*Cannon_black.size()-(w[3]+mod)*Cannon_white.size()) << endl;
				// 					cerr << ((w[4])+mod)*(hit_from_Cannon_Black())-((w[4])-mod)*(hit_from_Cannon_White()) << endl;
				// 					cerr << (w[7]+2*mod)*white_town_hall_attack_defence_utility()-(w[7]-2*mod)*black_town_hall_attack_defence_utility() << endl;
				// 					cerr << w[1]*(black_T.size()-white_T.size()) << endl;
				// 		}
				// 		else if(stratergy ==6){
				// 					cerr << ((w[0])-w[2]*color)*black_S.size() -((w[0])+w[2]*color)*white_S.size() << endl;
				// 					cerr << (w[3]+color)*(Cannon_black.size()-(w[3]-color)*Cannon_white.size()) << endl;
				// 					cerr << ((w[4])-color)*hit_from_Cannon_Black()-((w[4])+color)*(hit_from_Cannon_White()) << endl;
				// 					cerr << (w[7]-color)*white_town_hall_attack_defence_utility()-(w[7]+color)*black_town_hall_attack_defence_utility() << endl;
				// 					cerr << w[1]*(black_T.size()-white_T.size()) << endl;
				// 		}
				// 		else if(stratergy==7){
				...
				// 					cerr << w[3]*(Cannon_black.size()-Cannon_white.size()) << endl;
				// 					cerr << ((w[4])+mod)*(hit_from_Cannon_Black())-((w[4])-mod)*(hit_from_Cannon_White()) << endl;
				// 					cerr << (w[7]+mod)*white_town_hall_attack_defence_utility()-(w[7]-mod)*black_town_hall_attack_defence_utility() << endl;
				// 					cerr << w[1]*(black_T.size()-white_T.size()) << endl;
				//
				// 		}
				// 		else{
...
pair<float,Move> nextMove(State st,int depth,Move mv,float alpha,float beta,int limit){
    	numnodes++;
   		if(mv.init_pos!=mv.final_pos&&(depth==limit||st.white_T.size()==(M/2-2)||st.black_T.size()==(M/2-2))){
    		float t= st.utility();
...
int main(int argc, char **argv){
    ...
    while(true){
        	...
        	if(turn==color)
        	{
	      		...
	      		if(steps<6){
	        			stratergy = 1;
						update_weights(40,1000,5,6,5,4,4,6);
						}
	      		else if(total_sol>=18&&diff_sol<3){
	        			stratergy = 2;
						update_weights(40,1000,3,6,10,8,4,6);
	      		}
	      		else if(total_sol>=18&&diff_sol>=3){
						update_weights(40,1000,7,6,12,6,4,6);
	        			stratergy = 3;
	      		}
	      		else if(total_sol>=12&&diff_sol<3){
						update_weights(40,1000,3,8,14,4,4,6);
	        			stratergy = 4;
	      		}
	      		else if(total_sol>=12&&diff_sol>=3){
						update_weights(40,1000,3,6,12,4,4,6);
	        			stratergy = 5;
	      		}
	      		else if(total_sol>=8&&diff_sol<1){
	      			stratergy = 6;
					update_weights(40,1000,3,6,12,4,4,7);
	      			depth_limit =6;
	      		}
	      		else if(total_sol>=8&&diff_sol>=1){
	      		  	stratergy = 7;
					update_weights(40,1000,7,7,12,4,4,8);
	      			depth_limit =6;
	      		}
	      		else{
	      			stratergy = 8;
					update_weights(40,1000,5,5,10,4,4,15);
	      			depth_limit =6;
	      		}
				if(total_time - duration_till_now < 20){
					depth_limit = 4;
				}
				else if(15+last_move_time>total_time-duration_till_now){
					depth_limit = 4;
				}
				if(total_time - duration_till_now < 10){
					depth_limit = 2;
				}
						// depth_limit =6;
